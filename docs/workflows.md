...
- `career-ops-kr ops-check`: `verify`, `audit-jobs`, saved live smoke report health를 한 번에 묶어 운영 체크를 수행함. live smoke report가 없으면 기본은 `skip`으로 처리하고, `--require-live-smoke`를 주면 그 자체를 실패로 간주함
...
11. `트래커` 페이지에서 상태와 메모를 정리합니다. `문제 있음`, `리포트 없음`, `이력서 없음`, `팔로업 overdue`, `팔로업 미설정`, `tracker 미연결` preset으로 attention item만 바로 좁힐 수 있습니다.
...
13. attention이 높은 항목은 `문제 항목 선택`, `overdue 오늘 준비`, `미설정 3일 준비`, `tracker 미연결 선택` quick prep 버튼으로 현재 보이는 row 선택과 bulk 값 미리 채우기까지 한 번에 이어갈 수 있습니다. 마지막 적용은 기존 bulk 버튼에서 직접 실행합니다.
14. 저장한 공고를 다시 볼 때는 `트래커` 목록에서 상세 화면으로 들어가 tracker 상태와 연결된 JD/report/context/HTML/PDF를 확인하고, 공고 URL이 있으면 그 자리에서 맞춤 이력서를 다시 생성합니다.
15. 상세 화면에는 `다음에 할 일`과 tracker/web drift 요약이 같이 보여서, 단순 조회가 아니라 다음 액션 결정 화면으로 사용합니다.
16. `홈`으로 돌아오면 최근 생성한 HTML/PDF 이력서와 preset 경로를 다시 바로 확인할 수 있습니다. 이 목록은 웹에서 만든 산출물과 CLI에서 만든 산출물을 함께 보여주고, 연결된 saved job이 있으면 그 공고의 `다음 액션`도 같이 보여줍니다.
17. `홈`은 recent live smoke 상태를 짧게 요약하고, `설정`은 target별 문제/dir/report 수를 자세히 보여줍니다.
18. 웹 화면은 검색, 저장, 팔로업 정리, tracker 정리, deterministic resume build 중심으로 사용합니다.
